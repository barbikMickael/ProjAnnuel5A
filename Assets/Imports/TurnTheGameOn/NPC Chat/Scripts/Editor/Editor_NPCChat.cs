	...
	public override void OnInspectorGUI(){
		...
		if (showGeneralSettings) {
			...
			if (EditorGUI.EndChangeCheck ())
				serializedObject.ApplyModifiedProperties ();
			if (showHelp)
				EditorGUILayout.LabelField ("If player is not assigned, will repeatedly search the scene for the named player Transform", editorSkin.customStyles [3]);
			if (chatComponent.autoFindPlayer == NPCChat.AutoFindPlayer.byName) {
				...
				if (EditorGUI.EndChangeCheck ())
					serializedObject.ApplyModifiedProperties ();
				if (showHelp)
					EditorGUILayout.LabelField ("Name of the Transform to be searched and assigned as Player", editorSkin.customStyles [3]);
			}
			///Chat On Collision
